...
def podiff(old_pofile, new_pofile):
    ...
    old_msgs = {((entry.msgid, entry.msgctxt, entry.msgid_plural)): entry for entry in old_pofile}
    new_msgs = {((entry.msgid, entry.msgctxt, entry.msgid_plural)): entry for entry in new_pofile}
...
def pprint_diff(diff):
    ...
    for msg in diff['changed']:
        ...
        if old.msgid_plural != new.msgid_plural:
            print("\tmsgid_plural old: %r\n\t       new: %r" % (old.msgid_plural, new.msgid_plural))
        ...
        if len(deleted_flags) > 0:
            print("\tDeleted flags: %s" % (",".join(sorted(deleted_flags))))
